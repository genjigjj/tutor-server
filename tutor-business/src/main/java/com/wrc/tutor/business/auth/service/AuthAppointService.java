...
//      检测老师是否审核通过
        ...
        if (teacher.getRealnameAuth() != 1) {
            throw new BusinessException("教师未实现实名认证，禁止预约");
        }
        if (teacher.getStudentAuth() == 0 && teacher.getTeacherAuth() == 0) { //
            throw new BusinessException("教师未实现大学生认证或者教师认证，禁止预约");
        }
...
//                channel.basicNack(message.getMessageProperties().getDeliveryTag(), false,true);
     ...
     * @return
     */
    public boolean paySuccessProcess(Long id) {
        //        验证是否有权限
        Appoint appoint = getBaseMapper().selectById(id);
        if (appoint == null) {
            throw new BusinessException("appointId不存在");
        }
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        Long userId = Long.valueOf((String)authentication.getPrincipal());
        if (appoint.getStudentId().compareTo(userId) != 0) {
...
//        验证此 appoint 的状态是否为待付款状态
        ...
        log.info("接收处理DLX队列当中的消息: {}, 当前时间: {}", id, LocalDateTime.now().toString());
